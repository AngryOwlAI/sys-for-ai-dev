#!/usr/bin/env python3
"""Validate bootstrap-development skill manifests with the Python standard library.
...
def validate_registry(data: Any, path: Path, repo_root: Path) -> tuple[list[Finding], set[str]]:
    ...
    for index, entry in enumerate(skills):
        ...
        elif str(skill_path).startswith((".agents/", ".codex/")):
            findings.append(
                Finding(
                    path,
                    kind,
                    f"{skill_id}: canonical_path must not point at a generated host binding",
                )
            )
        else:
        ...
        for shim_path in entry.get("compatibility_shims", []):
            if not isinstance(shim_path, str) or not shim_path.startswith(
                ...
                findings.append(
                    Finding(
                        path,
                        kind,
                        f"{skill_id}: compatibility_shims must point under a generated host binding root",
                    )
                )
            if shim_path == skill_path:
                findings.append(Finding(path, kind, f"{skill_id}: compatibility shim cannot be canonical_path"))
            elif isinstance(shim_path, str) and not (repo_root / shim_path).exists():
                findings.append(
                    Finding(path, kind, f"{skill_id}: compatibility shim does not exist: {shim_path}")
                )
...
def discover_default_paths(repo_root: Path) -> dict[str, list[Path]]:
    ...
    return {
        ...
        "manifest": sorted(
            repo_root.glob("development/bootstrap-agent/skills/*/skill.y*ml")
        ),
        "bundle": sorted(
            (
                repo_root
        ...
        "domain_pack": sorted(
            (
                repo_root / "development" / "bootstrap-agent" / "domain-packs"
            ).glob("*.y*ml")
        ),
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate canonical bootstrap-development skills without external packages."
    )
