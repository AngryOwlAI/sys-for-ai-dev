# `sys-for-ai` PRD Integration Plan: Core Format Profiles, Configuration-Control Memory, and Validation Contracts

**Document status:** Recommendation implementation plan
**Prepared for:** `AngryOwlAI/sys-for-ai-dev`
**Target project:** `sys-for-ai`
**Target PRDs:**
...
This plan is based on the current public state of `AngryOwlAI/sys-for-ai-dev`, the current public state of `AngryOwlAI/The-AEther-Flow`, and official format/runtime documentation for TOML, Python `tomllib`, JSON Schema, and the Python `jsonschema` validator.

### 2.1 `sys-for-ai` current PRD baseline

The Phase 0 PRD already establishes `sys-for-ai` as a domain-agnostic system development and management framework for AI agents. It states that the framework output is a governed target agentic system or implementation-ready package with roles, artifacts, requirements, architecture, verification hooks, operating assumptions, source-first memory rules, improvement loops, and maintenance obligations.[^sfa-p0]
...
### 2.2 `sys-for-ai` current Phase 1 implementation baseline
...
The current implementation scaffold under `sys-for-ai/` includes:
...
The pattern to generalize into `sys-for-ai` is this:
...
5. A JSON wiki should appear only if `sys-for-ai` later adopts ordinary JSON files as first-class memory/source artifacts, such as tool-call trace files, state snapshots, API fixtures, structured evaluation records, or object stores.
...
| `primary_role` | What this format is for inside `sys-for-ai`. |
...
```csv
cfg_pyproject,pyproject.toml,toml,python_package,controlled,implementation_initialization,tomllib_or_tomli,contract_sys_for_ai_config,"build_backend;packaging",false,all,,pending,pending,"Packaging/tool config"
cfg_sys_for_ai_example,configs/examples/sys_for_ai.example.toml,toml,framework_example,controlled,implementation_initialization,tomllib_or_tomli,contract_sys_for_ai_config,"docs;validators",false,example,,pending,pending,"Example framework configuration"
...
```csv
contract_agentjob,schemas/contracts/agentjob.schema.json,2020-12,yaml,agentjob,control_records/**/*.yaml,controlled,implementation_initialization,"sys-for-ai validate-jsonschema-contracts",,pending,pending,"Validates parsed YAML AgentJob objects"
contract_handoff,schemas/contracts/handoff.schema.json,2020-12,yaml,handoff,control_records/**/*.yaml,controlled,implementation_initialization,"sys-for-ai validate-jsonschema-contracts",,pending,pending,"Validates parsed YAML handoff objects"
contract_completion_receipt,schemas/contracts/completion_receipt.schema.json,2020-12,yaml,completion_receipt,control_records/**/*.yaml,controlled,implementation_initialization,"sys-for-ai validate-jsonschema-contracts",,pending,pending,"Validates parsed YAML completion receipt objects"
contract_state_snapshot,schemas/contracts/state_snapshot.schema.json,2020-12,yaml,state_snapshot,control_records/**/*.yaml,controlled,implementation_initialization,"sys-for-ai validate-jsonschema-contracts",,pending,pending,"Validates parsed YAML bounded state snapshots"
contract_sys_for_ai_config,schemas/contracts/sys_for_ai_config.schema.json,2020-12,toml,framework_config,"pyproject.toml;configs/examples/sys_for_ai.example.toml",controlled,implementation_initialization,"sys-for-ai validate-toml-config",,pending,pending,"Validates parsed TOML framework/package config subset"
contract_target_project_config,schemas/contracts/target_project_config.schema.json,2020-12,toml,target_project_config,configs/examples/target_project.example.toml,controlled,implementation_initialization,"sys-for-ai validate-toml-config",,pending,pending,"Validates parsed target project config template"
contract_format_profile_registry_row,schemas/contracts/format_profile_registry_row.schema.json,2020-12,csv,format_profile_registry_row,registries/format_profile_registry.csv,controlled,implementation_initialization,"sys-for-ai validate-format-profiles",,pending,pending,"Validates format profile registry rows"
contract_config_source_registry_row,schemas/contracts/config_source_registry_row.schema.json,2020-12,csv,config_source_registry_row,registries/config_source_registry.csv,controlled,implementation_initialization,"sys-for-ai validate-config-sources",,pending,pending,"Validates config source registry rows"
contract_control_record_registry_row,schemas/contracts/control_record_registry_row.schema.json,2020-12,csv,control_record_registry_row,registries/control_record_registry.csv,controlled,implementation_initialization,"sys-for-ai validate-control-records",,pending,pending,"Validates control record registry rows"
contract_validation_contract_registry_row,schemas/contracts/validation_contract_registry_row.schema.json,2020-12,csv,validation_contract_registry_row,registries/validation_contract_registry.csv,controlled,implementation_initialization,"sys-for-ai validate-validation-contract-registry",,pending,pending,"Validates validation contract registry rows"
```
...
#### `configs/examples/sys_for_ai.example.toml`
...
source_artifacts:
  ...
  - source_id: SRC-PRD-P1
    path: PRDs/sys-for-ai_phase-1_implementation_initialization_prd.md
format_profile_evidence:
  governed_artifacts_read:
...
```json
{
  ...
  "$id": "https://example.invalid/sys-for-ai/schemas/contracts/agentjob.schema.json",
  "title": "sys-for-ai AgentJob",
  "type": "object",
  "required": [
    "agentjob_id",
    "objective",
...
#### Minimal `sys_for_ai_config.schema.json`

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://example.invalid/sys-for-ai/schemas/contracts/sys_for_ai_config.schema.json",
  "title": "sys-for-ai framework configuration",
  "type": "object",
...
```bash
sys-for-ai validate-format-profiles
sys-for-ai validate-config-sources
sys-for-ai validate-control-records
sys-for-ai validate-validation-contract-registry
sys-for-ai validate-toml-config
sys-for-ai validate-jsonschema-contracts
sys-for-ai validate-registry-graph
sys-for-ai generate-config-control-wiki
sys-for-ai generate-validation-contracts-catalog
sys-for-ai validate-generated-derivatives
```
...
```text
sys-for-ai/configs/README.md
sys-for-ai/configs/examples/sys_for_ai.example.toml
sys-for-ai/configs/examples/target_project.example.toml
sys-for-ai/schemas/contracts/*.schema.json
sys-for-ai/docs/format_profile_policy.md
sys-for-ai/docs/configuration_control_wiki_policy.md
sys-for-ai/docs/validation_contracts_catalog_policy.md
...
sys-for-ai/docs/generated/configuration_control/toml-configuration-sources.md
sys-for-ai/docs/generated/validation_contracts/index.md
sys-for-ai/docs/generated/validation_contracts/contracts-by-target.md
...
JSON Schema is a core validation-contract format, not a core wiki format. `sys-for-ai` SHALL generate a Validation Contracts Catalog rather than a JSON wiki. A JSON wiki MAY be introduced later only if JSON files become first-class source/memory artifacts beyond validation contracts, such as trace files, tool-call records, evaluation objects, API fixtures, or structured state snapshots.
...
## Decision
`sys-for-ai` will define Markdown, CSV, YAML, TOML, and JSON Schema as initial core file-format memory profiles.
...
1. `sys-for-ai` has a source-first memory system with explicit file-format profiles.
...
[^sfa-p0]: `sys-for-ai` Phase 0 PRD, `PRDs/sys-for-ai_phase-0_product_system_design_prd.md`, current public repository: https://github.com/AngryOwlAI/sys-for-ai-dev/blob/main/PRDs/sys-for-ai_phase-0_product_system_design_prd.md

[^sfa-p1]: `sys-for-ai` Phase 1 PRD, `PRDs/sys-for-ai_phase-1_implementation_initialization_prd.md`, current public repository: https://github.com/AngryOwlAI/sys-for-ai-dev/blob/main/PRDs/sys-for-ai_phase-1_implementation_initialization_prd.md

[^sfa-scaffold]: `sys-for-ai` scaffold root and README, current public repository: https://github.com/AngryOwlAI/sys-for-ai-dev/tree/main/sys-for-ai

[^sfa-pyproject]: `sys-for-ai/pyproject.toml` and `sys-for-ai/requirements.txt`, current public repository: https://github.com/AngryOwlAI/sys-for-ai-dev/tree/main/sys-for-ai

[^sfa-makefile]: `sys-for-ai/Makefile`, current public repository: https://github.com/AngryOwlAI/sys-for-ai-dev/blob/main/sys-for-ai/Makefile

[^sfa-cli]: `sys-for-ai/sys_for_ai/cli.py`, current public repository: https://github.com/AngryOwlAI/sys-for-ai-dev/blob/main/sys-for-ai/sys_for_ai/cli.py

[^sfa-validators]: `sys-for-ai/sys_for_ai/validators.py` and `sys-for-ai/sys_for_ai/memory.py`, current public repository: https://github.com/AngryOwlAI/sys-for-ai-dev/tree/main/sys-for-ai/sys_for_ai
