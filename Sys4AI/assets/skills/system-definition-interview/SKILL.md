...
Use this portable Sys4AI product asset to establish or reconstruct the intent of a target agentic system before downstream PRD, SRD, architecture, implementation-plan, or formal systems-document generation.
...
- Optional output path. Default: `requirements/discovery.md` when authorized.
...
18. Route unresolved material:
    - `decision-grilling` for unresolved scope or design choices.
    - `domain-grilling-with-docs` for terminology, glossary, documentation, or ADR-worthy conflicts.
...
```bash
sys4ai validate TARGET_WORKSPACE
```
...
Adapted from `AngryOwlAI/ai-skills-for-sys/skills/system-definition-interview` as a portable Sys4AI product asset. Upstream template behavior is preserved where compatible with `Sys4AI` source-first authority, explicit project authority, registry, and validation rules.
...
5. Change the catalog version only after review and validation evidence.
