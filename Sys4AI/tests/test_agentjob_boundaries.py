...
class AgentJobBoundaryTests(unittest.TestCase):
    def test_allowed_path_passes_from_repo_root(self) -> None:
        payload = validate_agentjob_boundaries(
            ...
            root="..",
            changed_paths=["Sys4AI/sys_for_ai/control_loop/boundaries.py"],
        )

        self.assertTrue(payload["ok"], payload)
        self.assertEqual(["Sys4AI/sys_for_ai/control_loop/boundaries.py"], payload["allowed"])
    ...
    def test_generated_derivative_requires_generated_path_authorization(self) -> None:
        payload = validate_agentjob_boundaries(
            "AJ-P1-BOUNDARY-VALIDATORS-001",
            changed_paths=["Sys4AI/docs/generated/configuration_control/index.md"],
        )
    ...
    def test_git_collection_includes_untracked_files(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (repo / "Sys4AI/tmp").mkdir(parents=True)
            (repo / "Sys4AI/tmp/new.txt").write_text("new\n", encoding="utf-8")

            payload = validate_agentjob_boundaries("AJ-TEMP-BOUNDARY-001", root=repo, use_git=True)

            self.assertTrue(payload["ok"], payload)
            self.assertIn("Sys4AI/tmp/new.txt", payload["changed_paths"])

    def test_validate_check_diff_current_repo_has_no_boundary_violations(self) -> None:
        payload = validate_check_diff("AJ-SYS4AI-DEV-NAME-MIGRATION-001")
        self.assertTrue(payload["ok"], payload)
...
def _write_temp_git_fixture(repo: Path) -> None:
    ...
    (repo / "Sys4AI/registries").mkdir(parents=True)
    (repo / "Sys4AI/control_records/agentjobs").mkdir(parents=True)
    (repo / "Sys4AI/registries/agentjob_registry.csv").write_text(
    ...
    dump_yaml(
        repo / "Sys4AI/control_records/agentjobs/AJ-TEMP-BOUNDARY-001.yaml",
        {
